...
const detectUserRole = (user, sessionServiceRef) => {
  try {
    const fromContext = user?.role;
    const fromSession = sessionServiceRef?.getUser?.()?.role;
    const role = (fromContext || fromSession || 'member').toString().toLowerCase();
    return role;
  } catch {
    return 'member';
  }
